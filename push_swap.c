...
//se o argc for superior nao precisamos do split porque ja vamos ter varios argumentos 
criacao do stack com os valores dados no comando - funcao void inicial_stack(stack a, argv) ?
   // funcao atol para converter uma string num long int ?
    Mensagem “Error” em caso de duplicados, maior ou menor que o integer (INT_MAX e INT_MIN), nao ser um integer
    funcao para anexar um node - ligar a head com o primeiro node e depois os seguintes pelo last node (ver funcao append new node e findlastnode)
...
algoritmo para 2, 3 , 4 e 5 numeros ?
             caso espeficicos 3 numeros usar so o stack a, passar o numero maior p o fim e depois verificar se os outros dois estao ordenados
algoritmo > 5  ---> 1. puxar todos os elementos p b ate ter apenas 5 na stack a;
                    2. e depois usar a funcao para se tiver 5 numeros;
                    3. encontrar o BBF de cada numero da stack a com cada da stack b (Nr A tem de ser > que o da B);
                    4. ver qual o custo (numero de comandos p o BBF topo da stack A e B e depois p a outra);
